...
rm -rf store_publisher store_provider store_client test_input.dat test_output.dat test_output_dht.dat pub_key.bin
...
./bin/publisher -p 45001 -ws-port 45002 -identity ./store_publisher/identity.key -store ./store_publisher -file test_input.dat --key-out ./pub_key.bin > publisher.log 2>&1 &
PUB_PID=$!
...
KEY_FINGERPRINT=$(grep "^Key Fingerprint" publisher.log | awk '{print $NF}')
PUB_ADDR=$(grep "127.0.0.1/tcp/45001/p2p/" publisher.log | head -n 1 | awk '{print $NF}')

if [ -z "$CONTENT_ID" ] || [ ! -f ./pub_key.bin ] || [ -z "$PUB_ADDR" ]; then
    echo "Error: Failed to parse publisher parameters from log or key file missing:"
    cat publisher.log
    exit 1
fi
...
./bin/client -p 55001 -ws-port 55002 -identity ./store_client/identity.key -store ./store_client -d "$PUB_ADDR" -fetch "$CONTENT_ID" --key-file ./pub_key.bin -out test_output.dat > client.log 2>&1
...
./bin/client -p 55010 -ws-port 55011 -identity ./store_client2/identity.key -store ./store_client2 -bootstrap "$BOOT_ADDR" -fetch "$CONTENT_ID" --key-file ./pub_key.bin -out test_output_dht.dat > client_dht.log 2>&1
