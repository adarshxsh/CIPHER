...
./bin/publisher -p 45001 -ws-port 45002 -identity ./store_publisher/identity.key -store ./store_publisher -export-key-file ./store_publisher/export.key -file test_input.dat > publisher.log 2>&1 &
PUB_PID=$!
...
if [ -z "$CONTENT_ID" ] || [ ! -f ./store_publisher/export.key ] || [ -z "$PUB_ADDR" ]; then
...
echo "  - Key file:  ./store_publisher/export.key"
echo "  - Address:   $PUB_ADDR"

echo "[4/5] Running Client to fetch, verify, and reassemble content..."
./bin/client -p 55001 -ws-port 55002 -identity ./store_client/identity.key -store ./store_client -d "$PUB_ADDR" -fetch "$CONTENT_ID" -key-file ./store_publisher/export.key -out test_output.dat > client.log 2>&1
...
./bin/client -p 55010 -ws-port 55011 -identity ./store_client2/identity.key -store ./store_client2 -bootstrap "$BOOT_ADDR" -fetch "$CONTENT_ID" -key-file ./store_publisher/export.key -out test_output_dht.dat > client_dht.log 2>&1
