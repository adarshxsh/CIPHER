...
./bin/publisher -p 45001 -ws-port 45002 -identity ./store_publisher/identity.key -store ./store_publisher -key-out ./store_publisher/content.key -file test_input.dat > publisher.log 2>&1 &
PUB_PID=$!
...
if [ -z "$CONTENT_ID" ] || [ -z "$KEY_FILE" ] || [ -z "$PUB_ADDR" ]; then
