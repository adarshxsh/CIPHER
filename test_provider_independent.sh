...
rm -f test_orig.dat test_recov1.dat test_recov2.dat pub_key.bin
...
./bin/publisher -seed=false -identity ./store_provider/pub.key -store ./store_provider -file test_orig.dat --key-out ./pub_key.bin > publisher.log 2>&1
...
./bin/client -p 48020 -ws-port 0 -identity ./store_client1/client.key -store ./store_client1 -bootstrap "$BOOT_ADDR" -fetch "$CONTENT_ID" --key-file ./pub_key.bin -out test_recov1.dat > client1.log 2>&1
...
./bin/client -p 48030 -ws-port 0 -identity ./store_client2/client.key -store ./store_client2 -bootstrap "$BOOT_ADDR" -fetch "$CONTENT_ID" --key-file ./pub_key.bin -out test_recov2.dat > client2.log 2>&1
