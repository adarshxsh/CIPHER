...
rm -f bootstrap.log publisher.log provider.log client1.log client2.log key.txt
...
./bin/publisher -seed=false -identity ./store_provider/pub.key -store ./store_provider -file test_orig.dat -key-out key.txt > publisher.log 2>&1

CONTENT_ID=$(grep "^ContentID" publisher.log | awk '{print $NF}')

echo "Content Published:"
echo "  - ContentID:      $CONTENT_ID"
echo "  - Key Exported:   key.txt"
...
./bin/client -p 48020 -ws-port 0 -identity ./store_client1/client.key -store ./store_client1 -bootstrap "$BOOT_ADDR" -fetch "$CONTENT_ID" -key-file key.txt -out test_recov1.dat > client1.log 2>&1
...
./bin/client -p 48030 -ws-port 0 -identity ./store_client2/client.key -store ./store_client2 -bootstrap "$BOOT_ADDR" -fetch "$CONTENT_ID" -key-file key.txt -out test_recov2.dat > client2.log 2>&1
