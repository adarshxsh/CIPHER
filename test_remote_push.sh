...
rm -f bootstrap.log provider1.log provider2.log provider3.log publisher.log client1.log client2.log key.txt
...
./bin/publisher -p 48040 -ws-port 0 -file test_orig.dat -store ./store_pub \
    ...
    -bootstrap "$BOOT_ADDR" -seed=false -key-out key.txt > publisher.log 2>&1
...
./bin/client -p 48050 -ws-port 0 -identity ./store_client1/client.key -store ./store_client1 \
    -bootstrap "$BOOT_ADDR" -fetch "$CONTENT_ID" -key-file key.txt -out test_recovered.dat > client1.log 2>&1
...
./bin/client -p 48060 -ws-port 0 -identity ./store_client2/client.key -store ./store_client2 \
    -bootstrap "$BOOT_ADDR" -fetch "$CONTENT_ID" -key-file key.txt -out test_recovered_fault.dat > client2.log 2>&1
