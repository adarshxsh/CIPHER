#!/bin/bash
cd p2p_transport
rm -rf store_a store_b test.mp4 out.mp4 transfer.key peer_a.log
export CGO_ENABLED=0
go build -o bin/peer ./cmd/peer

echo "Testing plaintext transfer..." > test.mp4

./bin/peer -p 47891 -ws-port 0 -identity ./store_a/identity.key -store ./store_a -ingest test.mp4 -key-out transfer.key > peer_a.log 2>&1 &
PEER_A_PID=$!
