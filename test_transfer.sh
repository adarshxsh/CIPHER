...
./bin/peer -p 47891 -ws-port 0 -identity ./store_a/identity.key -store ./store_a -ingest test.mp4 -key-file ./store_a/exported.key > peer_a.log 2>&1 &
PEER_A_PID=$!

sleep 3

CONTENT_ID=$(grep -a "ContentID:" peer_a.log | awk '{print $NF}')
KEY=$(cat ./store_a/exported.key | tr -d '\r\n')
