...
func (h *Header) WriteTo(w io.Writer) error {
	...
	if uint16(len(filenameBytes)) > MaxFilenameSize || len(filenameBytes) > int(MaxFilenameSize) {
		return fmt.Errorf("filename length %d exceeds maximum allowed size (%d)", len(filenameBytes), MaxFilenameSize)
	}
...
func (h *Header) ReadFrom(r io.Reader) error {
	...
	if filenameLen > MaxFilenameSize {
		return fmt.Errorf("filename length %d exceeds maximum allowed size (%d)", filenameLen, MaxFilenameSize)
	}
