...
func Send(s network.Stream, filePath string) error {
	...
	// 1. Construct and Write Header (ProtocolVersion2)
	header := &Header{
	...
	// 2. Send Data with In-Stream Hashing and Progress Tracking
	...
	// 3. Transmit Trailing Checksum Frame
	checksum := hasher.Sum(nil)
	if _, err := s.Write(checksum); err != nil {
		return fmt.Errorf("failed to send trailing checksum frame: %w", err)
	}
