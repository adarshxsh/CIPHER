...
func Send(s network.Stream, filePath string) error {
	...
	// 1. Construct and Write Header (Checksum placeholder set to zero array)
	header := &Header{
	...
	// 2. Send Data in Single Pass with Concurrent In-Stream Hashing
	...
	// 3. Append 32-byte Trailer SHA-256 Checksum
	var trailerChecksum [32]byte
	copy(trailerChecksum[:], hasher.Sum(nil))

	if _, err := s.Write(trailerChecksum[:]); err != nil {
		return fmt.Errorf("failed to write trailer checksum: %w", err)
	}
