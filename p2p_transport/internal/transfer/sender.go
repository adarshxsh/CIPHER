...
func Send(s network.Stream, filePath string) error {
	...
	// 1. Construct and Write Header with zeroed Checksum (trailing checksum mode)
	header := &Header{
	...
	// 2. Send Data with Progress Tracking and Inline Checksum Calculation (Single Pass)
	...
	// 3. Write trailing SHA-256 checksum after streaming file payload
