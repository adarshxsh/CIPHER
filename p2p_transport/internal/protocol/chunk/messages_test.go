...
func TestProtocolCompatibility_OldDecoder(t *testing.T) {
	// A new version comes in; ReadMessage now enforces version check via DecodeFrame header pre-validation
	msg := &chunk.Message{
...
func TestProtocolCompatibility_MalformedMessage(t *testing.T) {
	// Payload smaller than required for REQUEST_MANIFEST
	msg := &chunk.Message{
...
func TestReadMessage_PreValidatesHeaderAndRejectsOversizedPayload(t *testing.T) {
	var buf bytes.Buffer

	// Control message (MsgRequestManifest) with declared size 1000 (> MaxChunkRequestSize 512 bytes)
