...
func TestProtocolCompatibility_OldDecoder(t *testing.T) {
	// A new version comes in, ReadMessage delegates framing checks to DecodeFrame
	// and strictly rejects unsupported versions.
	msg := &chunk.Message{
...
func TestProtocolCompatibility_MalformedMessage(t *testing.T) {
	// Payload parser should reject a payload that is too short
	msg := &chunk.Message{
...
func TestProtocolCompatibility_UnsupportedMessage(t *testing.T) {
	// ReadMessage delegates to DecodeFrame which rejects unknown message types
	msg := &chunk.Message{
		Version: chunk.CurrentMessageVersion,
...
func TestReadMessage_OversizedPayloadRejected(t *testing.T) {
	var buf bytes.Buffer
