...
func (c *Client) Resolve(ctx context.Context, id core.ContentID) ([]byte, error) {
	if err := c.stream.SetWriteDeadline(time.Now().Add(DefaultWriteDeadline)); err != nil {
	...
	if err := c.stream.SetReadDeadline(time.Now().Add(DefaultReadDeadline)); err != nil {
...
func (c *Client) FetchChunk(ctx context.Context, chunkID core.ChunkID) (*core.Chunk, error) {
	...
	if err := c.stream.SetWriteDeadline(time.Now().Add(DefaultWriteDeadline)); err != nil {
		_ = c.stream.SetDeadline(time.Time{})
		return nil, fmt.Errorf("failed to set ACK write deadline: %w", err)
	}
