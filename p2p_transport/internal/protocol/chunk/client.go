...
type Client struct {
	...
	mu            sync.Mutex
	closed        bool
	activeStreams map[network.Stream]struct{}
}
...
func NewClient(ctx context.Context, t *transport.Transport, peerID peer.ID, eng *engine.ContentEngine) (*Client, error) {
	...
	return &Client{
		...
		activeStreams: make(map[network.Stream]struct{}),
	}, nil
}
...
func (c *Client) removeStream(stream network.Stream) {
	c.mu.Lock()
...
func (c *Client) Close() error {
	...
	c.closed = true
	streams := make([]network.Stream, 0, len(c.activeStreams))
	for s := range c.activeStreams {
		streams = append(streams, s)
	}
	c.activeStreams = make(map[network.Stream]struct{})
	c.mu.Unlock()
