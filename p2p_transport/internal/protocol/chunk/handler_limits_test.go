...
func TestHandler_TransactionLimitExceeded(t *testing.T) {
	...
	if err := chunk.WriteMessage(s, req2); err != nil {
		// Stream might already be closed by server after transaction 1
	}

	// Reading response for transaction 2 should fail because server closed stream after transaction 1
	_, err = chunk.ReadMessage(s)
	if err == nil {
		t.Fatalf("Expected error when attempting transaction 2 over single-transaction stream, got nil")
	}
}

func TestHandler_AckReadDeadlineExceeded(t *testing.T) {
	origTimeout := chunk.StreamTimeout
	chunk.StreamTimeout = 100 * time.Millisecond
	defer func() { chunk.StreamTimeout = origTimeout }()
