...
func (h *StreamHandler) handleStream(s network.Stream) {
	...
	for {
		if txCount >= MaxTransactionsPerStream {
			log.Printf("[Chunk Protocol] Stream transaction limit reached (%d >= %d)", txCount, MaxTransactionsPerStream)
			return
		}
		if msgCount >= MaxMessagesPerStream {
			log.Printf("[Chunk Protocol] Stream message limit reached (%d >= %d)", msgCount, MaxMessagesPerStream)
			return
		}
		...
		if msgCount > MaxMessagesPerStream {
			log.Printf("[Chunk Protocol] Exceeded max messages per stream: %d > %d", msgCount, MaxMessagesPerStream)
			WriteMessage(s, BuildError(ErrBadRequest, "message limit exceeded"))
			return
		}

		if txCount >= MaxTransactionsPerStream {
			log.Printf("[Chunk Protocol] Exceeded max transactions per stream: %d >= %d", txCount, MaxTransactionsPerStream)
...
func (h *StreamHandler) handleRequestChunk(s network.Stream, msg *Message, msgCount *int) {
	...
	*msgCount++
	if *msgCount > MaxMessagesPerStream {
		log.Printf("[Chunk Protocol] Exceeded max messages per stream while reading ACK: %d > %d", *msgCount, MaxMessagesPerStream)
		WriteMessage(s, BuildError(ErrBadRequest, "message limit exceeded"))
		return
	}
