...
func (h *StreamHandler) handleStream(s network.Stream) {
	...
	for {
		...
		if msgCount > MaxMessagesPerStream {
			log.Printf("[Chunk Protocol] Message count limit (%d) exceeded on stream from %s", MaxMessagesPerStream, s.Conn().RemotePeer())
			WriteMessage(s, BuildError(ErrBadRequest, "message limit exceeded"))
			return
		}

		if txCount >= MaxTransactionsPerStream {
			log.Printf("[Chunk Protocol] Transaction limit (%d) exceeded on stream from %s", MaxTransactionsPerStream, s.Conn().RemotePeer())
		...
		if txCount >= MaxTransactionsPerStream {
			log.Printf("[Chunk Protocol] Completed max transactions (%d), closing stream", MaxTransactionsPerStream)
			return
		}
	}
}
...
func (h *StreamHandler) handleRequestChunk(s network.Stream, msg *Message, msgCount *int) {
	...
	*msgCount++
	if *msgCount > MaxMessagesPerStream {
		log.Printf("[Chunk Protocol] Message count limit (%d) exceeded on stream from %s while waiting for ACK", MaxMessagesPerStream, s.Conn().RemotePeer())
		WriteMessage(s, BuildError(ErrBadRequest, "message limit exceeded"))
		return
	}
