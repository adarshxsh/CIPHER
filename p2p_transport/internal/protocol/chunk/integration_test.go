...
func TestChunkProtocol_RejectsTamperedManifest(t *testing.T) {
	h1, h2 := setupMockNetwork(t)
	eng1 := createTestEngine(t)
	eng2 := createTestEngine(t)
	chunk.NewStreamHandler(h1, eng1)
	...
	ctx := context.Background()
	data := []byte("some payload data to ingest")
	m, err := eng1.Ingest(ctx, bytes.NewReader(data), manifest.TypeFile)
	if err != nil {
		t.Fatalf("Ingest failed: %v", err)
	}

	// Store a tampered manifest under m.Descriptor.ID in eng1's store
	mTampered := *m
	...
	// Client attempts to resolve manifest from Peer 1
	client, err := chunk.NewClient(ctx, transport.NewTransport(h2), h1.ID(), eng2)
	if err != nil {
		t.Fatalf("Failed to create client: %v", err)
	}
	...
	if err == nil {
		t.Fatalf("Expected Client.Resolve to fail for tampered manifest payload, got nil")
	}
}
