...
func ReadMessage(r io.Reader) (*Message, error) {
	...
	size := binary.LittleEndian.Uint32(header[:frameLengthSize])
	...
	if size > MaxFrameSize {
		return nil, fmt.Errorf("%w: message exceeds maximum frame size", ErrPayloadTooLarge)
	}

	version := binary.LittleEndian.Uint16(header[frameLengthSize : frameLengthSize+2])
	msgType := MessageType(header[frameLengthSize+2])
