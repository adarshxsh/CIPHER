...
func TestContentEngine_EndToEnd(t *testing.T) {
	...
	if m.Crypto.Algorithm != "XChaCha20-Poly1305" {
		t.Errorf("expected manifest crypto algorithm XChaCha20-Poly1305, got %s", m.Crypto.Algorithm)
	}

	if m.Crypto.ChunkNonceSize != 24 {
		t.Errorf("expected manifest crypto ChunkNonceSize 24, got %d", m.Crypto.ChunkNonceSize)
	}
