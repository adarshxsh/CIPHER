...
func (e *ContentEngine) Ingest(ctx context.Context, r io.Reader, mtype manifest.ContentType) (*manifest.Manifest, error) {
	...
	// Derive ContentID strictly as the SHA-256 digest of normalized manifest JSON content
	contentID, _, err := m.CalculateContentID()
	if err != nil {
		return nil, fmt.Errorf("failed to calculate manifest ContentID: %w", err)
	}
