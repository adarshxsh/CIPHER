...
// Reassemble reads the manifest, fetches chunks, decrypts them, verifies integrity, and writes to w sequentially.
func (e *ContentEngine) Reassemble(ctx context.Context, m *manifest.Manifest, w io.Writer) error {
	...
	for _, chunkID := range m.ChunkIDs {
		...
		// Write decrypted chunk payload directly to target writer
		if _, err := w.Write(chunk.Data); err != nil {
			return fmt.Errorf("failed to write decrypted chunk: %w", err)
		}

		// Release memory immediately to maintain constant O(1) resident RAM footprint
		chunk.Data = nil
		chunk = nil
	}
