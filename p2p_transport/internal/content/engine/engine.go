...
func (e *ContentEngine) Ingest(ctx context.Context, r io.Reader, mtype manifest.ContentType) (*manifest.Manifest, error) {
	...
	m := &manifest.Manifest{
		...
		Descriptor: manifest.ContentDescriptor{
			ID:   core.ContentID{}, // Zeroed for deterministic hash computation
			Type: mtype,
			Size: totalSize,
		},
		ChunkIDs:   chunkIDs,
	...
	// Store key under computed ContentID
	if err := e.keys.Put(ctx, contentID, key); err != nil {
		return nil, fmt.Errorf("failed to store key: %w", err)
	}

	// Store manifest bytes in manifest store if available
	if e.manifestStore != nil {
		if mBytes, err := m.Serialize(); err == nil {
			_ = e.manifestStore.PutManifestBytes(ctx, contentID, mBytes)
		}
	}
