...
func (e *ContentEngine) Ingest(ctx context.Context, r io.Reader, mtype manifest.ContentType) (*manifest.Manifest, error) {
	...
	// Compute ContentID as SHA-256 digest of canonical manifest bytes
	manifestBytes, err := m.Serialize()
	if err != nil {
		return nil, fmt.Errorf("failed to serialize manifest: %w", err)
	}

	contentHash := e.digest.Sum(manifestBytes)
	...
	// Store manifest
	if err := e.manifestStore.PutManifestBytes(ctx, contentID, manifestBytes); err != nil {
