...
// Reassemble reads the manifest, fetches chunks sequentially, decrypts them, verifies integrity, and writes to w.
func (e *ContentEngine) Reassemble(ctx context.Context, m *manifest.Manifest, w io.Writer) error {
	...
	// Fetch, verify, decrypt, and write each chunk sequentially
	for _, chunkID := range m.ChunkIDs {
		...
		// Write out directly to target writer
		if _, err := w.Write(chunk.Data); err != nil {
			return fmt.Errorf("failed to write decrypted chunk: %w", err)
		}

		// Clear chunk reference immediately prior to fetching the next chunk
		chunk = nil
	}
