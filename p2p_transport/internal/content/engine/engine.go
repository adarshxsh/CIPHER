...
// Reassemble reads the manifest, fetches chunks sequentially, decrypts them, verifies integrity, and streams directly to w.
func (e *ContentEngine) Reassemble(ctx context.Context, m *manifest.Manifest, w io.Writer) error {
	...
	// Fetch chunks sequentially, decrypt, verify, and write directly to stream
	for _, chunkID := range m.ChunkIDs {
		...
		// Stream decrypted payload directly to output
		if _, err := bw.Write(chunk.Data); err != nil {
			return fmt.Errorf("failed to write decrypted chunk: %w", err)
		}

		// Free chunk payload reference to maintain constant heap footprint
		chunk.Data = nil
		chunk = nil
	}
	...
	if err := bw.Flush(); err != nil {
		return fmt.Errorf("failed to flush reassembly writer: %w", err)
	}

	// Sync underlying file handle if supported
	type syncer interface {
		Sync() error
	}
	if s, ok := w.(syncer); ok {
		if err := s.Sync(); err != nil {
			return fmt.Errorf("failed to sync reassembly writer: %w", err)
		}
	}
