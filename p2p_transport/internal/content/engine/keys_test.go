...
func TestFSKeyProvider_ContextCanceled(t *testing.T) {
	tmpDir, err := os.MkdirTemp("", "fs-key-provider-ctx-*")
	if err != nil {
		t.Fatalf("failed to create temp dir: %v", err)
	}
	defer os.RemoveAll(tmpDir)
	...
	if err := kp.Put(ctx, id, key); !errors.Is(err, context.Canceled) {
		t.Errorf("expected context.Canceled on Put, got %v", err)
	}

	if _, err := kp.Get(ctx, id); !errors.Is(err, context.Canceled) {
		t.Errorf("expected context.Canceled on Get, got %v", err)
	}

	if err := kp.Delete(ctx, id); !errors.Is(err, context.Canceled) {
		t.Errorf("expected context.Canceled on Delete, got %v", err)
	}
}

func TestFSKeyProvider_Timeout(t *testing.T) {
	tmpDir, err := os.MkdirTemp("", "fs-key-provider-timeout-*")
	if err != nil {
		t.Fatalf("failed to create temp dir: %v", err)
	}
	defer os.RemoveAll(tmpDir)
	...
	var id core.ContentID
	if _, err := kp.Get(ctx, id); !errors.Is(err, context.DeadlineExceeded) {
