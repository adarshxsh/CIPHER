...
// It generates a 24-byte cryptographically secure random nonce via crypto/rand for every chunk encryption.
...
func (e *ChaCha20Encryptor) DecryptChunk(key []byte, chunk *core.Chunk) error {
	if len(chunk.Header.Nonce[:]) != chacha20poly1305.NonceSizeX {
		return errors.New("invalid nonce length for XChaCha20-Poly1305")
	}
