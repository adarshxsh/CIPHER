...
// WithConnectionLimits configures custom connection manager low and high watermarks and grace period.
func WithConnectionLimits(low, high int, grace time.Duration) NodeOption {
...
// WithConnLimits is an alias for WithConnectionLimits.
func WithConnLimits(low, high int, grace time.Duration) NodeOption {
	return WithConnectionLimits(low, high, grace)
}

// WithResourceLimits sets custom maximum memory limit in bytes.
func WithResourceLimits(mem int64) NodeOption {
	return func(o *nodeOptions) {
		o.maxMemory = mem
	}
}

// WithMemoryLimit is an alias for WithResourceLimits.
func WithMemoryLimit(mem int64) NodeOption {
	return WithResourceLimits(mem)
}

// WithCustomSwarmQuotas provides custom resource manager limit overrides.
func WithCustomSwarmQuotas(limits rcmgr.PartialLimitConfig) NodeOption {
	return func(o *nodeOptions) {
		o.partialLimits = &limits
	}
}

// WithPartialLimitConfig is an alias for WithCustomSwarmQuotas.
func WithPartialLimitConfig(limits rcmgr.PartialLimitConfig) NodeOption {
	return WithCustomSwarmQuotas(limits)
}
