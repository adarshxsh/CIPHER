...
func TestNewHostCustomOptions(t *testing.T) {
	...
	host, kdht, err := NewHost(
		ctx, 0, 0, nil, "", false,
		WithResourceLimits(512*1024*1024),
		WithConnectionLimits(10, 50, 30*time.Second),
		WithMetricsReporter(metrics),
		WithCustomSwarmQuotas(customLimits),
	)
	if err != nil {
...
func TestStreamLimitEnforcement(t *testing.T) {
	...
	hA, dhtA, err := NewHost(ctx, 0, 0, nil, "", false, WithCustomSwarmQuotas(smallLimits))
	if err != nil {
	...
	hB, dhtB, err := NewHost(ctx, 0, 0, nil, "", false, WithCustomSwarmQuotas(smallLimits))
	if err != nil {
