...
// signing the provider announcement with the node's Ed25519 private key.
func Provide(ctx context.Context, kdht *dht.IpfsDHT, id core.ContentID, privKeys ...crypto.PrivKey) error {
	var privKey crypto.PrivKey
	if len(privKeys) > 0 && privKeys[0] != nil {
		privKey = privKeys[0]
	} else if kdht != nil && kdht.Host() != nil && kdht.Host().Peerstore() != nil {
		privKey = kdht.Host().Peerstore().PrivKey(kdht.PeerID())
	}
...
func StartRepublisher(ctx context.Context, kdht *dht.IpfsDHT, store core.ManifestStore, interval time.Duration, privKeys ...crypto.PrivKey) {
	go func() {
		// Republish immediately on startup
		republishAll(ctx, kdht, store, privKeys...)
...
func republishAll(ctx context.Context, kdht *dht.IpfsDHT, store core.ManifestStore, privKeys ...crypto.PrivKey) {
	...
	for _, id := range manifests {
		if err := Provide(ctx, kdht, id, privKeys...); err != nil {
