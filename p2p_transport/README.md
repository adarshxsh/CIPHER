...
It natively supports NAT traversal by leveraging libp2p `circuitv2` public relays and automatically upgrades to high-speed direct TCP/UDP/WebSocket connections in the background via **DCUtR (Hole Punching)**.

## Structure

- `cmd/peer`: Entry point for standard peers (Supports TCP and WebSocket).
- `cmd/relay`: Entry point for relay peers (Supports TCP, QUIC, and WebSocket).
