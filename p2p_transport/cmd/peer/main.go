...
func main() {
	...
	offset := flag.Int("offset", 0, "Offset for paginated transfer session listing")
	limit := flag.Int("limit", 50, "Limit for paginated transfer session listing")
	...
	if *transferStatus {
		sessions, err := sm.ListPaged(*offset, *limit)
		if err != nil {
